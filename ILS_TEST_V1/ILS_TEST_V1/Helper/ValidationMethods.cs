...
namespace ILS_TEST_V1.Helper
{
    public class ValidationMethods
    {
        ...
        {
            DSPsdLayerVMList = new BindingList<layerModel>();
            DsValidCodeList = new BindingList<ValidateVM>();
            DsErrorMsgList = new BindingList<ErrorMsgVM>();
        }

        public BindingList<layerModel> DSPsdLayerVMList { get; set; }
        ...
        private IList<layerModel> GetChildLayerListByDefaultParentIndex<T>(int defaultLayerIndex, ref string parentLayerName, ref int parentLayerIndex)
        {
        ...
        private void M_JC_WD_AC_0001(ValidateVM item)
        {
            ...
            if (arrowLayerList.Count() >= 1)
            {
                ...
                foreach (var x in arrowLayerList)
                {
                    ...
                    if (match.Success == false)
                    {
                        AddErrorMsg(item, x.Index, string.Format("레이어명 오류 :  명칭 {0}", x.Name));
                    }
                }
            }
        ...
        private void M_JC_WD_LS_0002(ValidateVM item)
        {
            ...
            if (arrowLayerList.Count() >= 1)
            {
                ...
                foreach (var x in arrowLayerList)
                {
                    for (var layerSeq = 1; layerSeq <= maxLayerSeq; layerSeq++)
                    {
                        layerModel temp = DSPsdLayerVMList.Where(y => y.ParentIndex == x.Index && y.LayerSeq == layerSeq).FirstOrDefault();
                        if (temp == null)
                        {
                            AddErrorMsg(item, string.Format("레이어명 없음 : 부모순번 {0}, 순서 {1}", x.Index, layerSeq));
                        }
        ...
        private bool CheckLayer(ValidateVM item, BindingList<layerModel> list, LayerNameVM vm)
        {
        ...
        private void CheckLayerLocation(layerModel layer, ValidateVM item)
        {
